...
public:
    ...
    friend ostream& operator << (ostream& out, const Ext& obj);

    bool operator < (Ext obj) {
        if (this->neg && !obj.neg)
            return true;
        else if (!this->neg && obj.neg)
            return false;
        else if (this->neg)
            return this->n > obj.n;
        else
    ...
    bool operator == (Ext obj) {
        return (!(this->operator<(obj)) && !(obj < *this));
    }

    bool operator == (int i) {
        Ext tmp = Ext(Integer(abs(i)));
        if (i < 0)
            tmp.neg = true;
        return this->operator==(tmp);
    }

    Ext operator / (Ext obj) {
        Ext res = Ext(this->n / obj.n);
        if (this->neg ^ obj.neg) {
            res.neg = true;
        }
        return res;
    ...
    Ext operator % (Ext obj) {
        Ext tmp = obj.operator*(this->operator/(obj));
        //cout << "neg: " << tmp.neg << endl;
        //cout << "tmp: " << tmp.n;
        return this->operator-(tmp);
    }
};